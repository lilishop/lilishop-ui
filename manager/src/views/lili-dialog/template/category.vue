<template>
  <div class="wrapper">
    ...
    <div class="list">
      <div
        class="list-item"
        :class="{ active: parentIndex === cateIndex }"
        @click="handleClickChild(cate, cateIndex)"
    ...
    <!-- 二级分类 -->
    <div class="list" v-if="secondLevel.length != 0">
      <div
        class="list-item"
        :class="{ active: secondIndex === secondI }"
        @click="handleClickSecondChild(second, secondI)"
        v-for="(second, secondI) in secondLevel"
        :key="secondI"
      >
        {{ second.name }}
      </div>
    ...
    <!--三级分类 -->
    <div class="list" v-if="thirdLevel.length != 0">
      <div
        class="list-item"
        :class="{ active: thirdIndex === thirdI }"
        @click="handleClickthirdChild(third, thirdI)"
        v-for="(third, thirdI) in thirdLevel"
        :key="thirdI"
      >
        {{ third.name }}
      </div>
...
export default {
  ...
  methods: {
    ...
    handleClickthirdChild(item, index) {
      item.___type = "category";
      item.allId = `${this.categoryList[this.parentIndex].id},${
        item.parentId
      },${item.id}`;
      this.$emit("selected", [item]);
