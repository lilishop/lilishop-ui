...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      getSupplierListData(this.searchForm).then((res) => {
        this.loading = false;
        if (res.success) {
    ...
    // 添加供应商
    add() {
      this.$router.push({ path: '/supplier-operation'});
    },
    // 修改供应商
    edit(v) {
      this.$router.push({ path: '/supplier-operation', query: { supplierId: v.id } });
    },
    // 关闭供应商
    disable(v) {
      this.$Modal.confirm({
        title: "确认关闭",
        content: "您确认要关闭供应商 " + v.storeName + " ?",
        loading: true,
        onOk: () => {
          disableSupplier(v.id).then((res) => {
            this.$Modal.remove();
            if (res.success) {
    ...
    //查看供应商详细
    detail(row){
      this.$router.push({ name: "supplier-detail", query: { id: row.id } });
    },
    // 审核供应商
    audit(v) {
      this.$Modal.confirm({
        title: "审核供应商",
        content: "您确认要审核通过供应商 " + v.storeName + " ?",
        okText: "通过",
        cancelText: "驳回",
        loading: true,
        onOk: () => {
          supplierAudit(v.id, 0).then((res) => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.getDataList();
    ...
    enable(v) {
      this.$Modal.confirm({
        title: "确认开启",
        content: "您确认要开启供应商 " + v.storeName + " ?",
        loading: true,
        onOk: () => {
          enableSupplier(v.id).then((res) => {
            this.$Modal.remove();
            if (res.success) {
              this.$Message.success("操作成功");
              this.getDataList();
