...
<script>
  ...
  import * as API_Supplier from "@/api/supplier.js";
  import * as API_SupplierOrder from "@/api/supplierOrder.js";
  ...
  export default {
    ...
    methods: {
      ...
      //查询会员信息
      getStoreInfo() {
        API_Supplier.supplierDetail(this.id).then((res) => {
      ...
      shopStatusChange(v) {
        if (v) {
          API_Supplier.enableSupplier(this.id).then(res => {
          });
        } else {
          API_Supplier.disableSupplier(this.id).then(res => {
          });
        }
      },
      //查询TA的订单
      getOrderData() {
        ...
        API_SupplierOrder.getOrderList(this.orderSearchForm).then((res) => {
          this.loading = false;
          if (res.success) {
      ...
      getRefundOrder() {
        ...
        API_SupplierOrder.getAfterSaleOrderPage(this.refundOrderSearchForm).then((res) => {
          this.loading = false;
          if (res.success) {
      ...
      getRefundGoodsOrderData() {
        ...
        API_SupplierOrder.getAfterSaleOrderPage(this.refundGoodsOrderSearchForm).then((res) => {
          this.loading = false;
          if (res.success) {
